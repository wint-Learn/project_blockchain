...
async function loginWithSignature({ address, ip, userAgent, pool, contract, logger }) {
  ...
  // 2) Log login with geolocation and anomaly detection (this handles everything)
  await logLogin(pool, {
    userId: user.id,
  ...
  // 3) Log to user_logins for activity history
  try {
    await pool.query(
  ...
  logger.info('MetaMask login successful', { address, ip });
